...
public final class WorkspaceModel {
  ...
  public init(host: WorkspaceHost?, options: FolderScanner.Options, roots: BookmarkStore = .shared) {
    self.host = host
  ...
  /**
   The rename in progress, and what the user has typed so far.
   ...
   */
  public private(set) var renaming: URL?
  ...
  public func commitRenaming() {
    guard let url = renaming else {
      return
    }
  ...
  public func newFile(near url: URL) {
    let directory = node(for: url)?.isDirectory == true ? url : url.deletingLastPathComponent()
  ...
  public func chooseFolder() {
    Task { [weak self] in
      ...
      do {
        try roots.save(folder)
      } catch {
        // The folder cannot be remembered — deleted or unmounted between the panel and here.
        // Opening it for this launch is a better answer than acting as if nothing was chosen.
        roots.useWithoutSaving(folder)
      }
    }
  }
